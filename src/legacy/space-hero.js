...
// @ts-nocheck
/**
 * LEGACY — the pre-DOKKA space hero (starfield, cursor constellations, shooting
 * star, earth/moon orbit) and the WORLD BLUEPRINT panels that go with it.
 *
 * This is the previous site's homepage, kept verbatim and quarantined behind
 * flags.legacyHero so the live domain keeps working while the alley and the
 * workshop are built. It is excluded from typecheck and lint on purpose: it is
 * scheduled for deletion in STEP 2, not for maintenance.
 *
 * Do not import this from new code. Do not extend it.
 */
export function mountSpaceHero(){
  // Was closed over by the old top-level IIFE; re-declared here so the module
  // stands alone.
