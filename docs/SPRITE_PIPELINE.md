...
## 상태 구간은 캐릭터마다 다르다 (2026-09-13)

`scripts/states.js`의 `REGIONS`는 서 있는 키의 비율로 관절 위치를 적는다. 목을
압축하고 머리를 내리면서 키가 6% 줄었으므로, 같은 비율이 더 이상 같은 곳을 가리키지
않는다. 특히 `neckY 0.55`는 압축 후에는 머리 *안쪽*이라, 그대로 두면 `look`이 머리
윗부분만 돌려 턱에서 어긋난다.

`scripts/regions.json`은 캐릭터마다 같은 **물리적 지점**을 가리키도록 다시 계산한
값이다. 계산은 `hide_neck.py`의 변환을 그대로 역으로 적용한다: 밴드 아래는 그대로,
밴드 안은 25%로, 밴드 위는 내려간 만큼 빼고, 새 키로 나눈다. `render_states.html`이
이 파일을 읽고 없으면 기존 기본값으로 돌아간다.

| | legTop | hipY | shoulderY | neckY |
|---|---|---|---|---|
| momo | 0.3191 | 0.3511 | 0.4761 | 0.5213 |
| nunu | 0.3191 | 0.3511 | 0.4601 | 0.5213 |
| ruki | 0.3191 | 0.3511 | 0.4681 | 0.5213 |
| yomi | 0.3191 | 0.3511 | 0.4441 | 0.5213 |
| poko | 0.3243 | 0.3568 | 0.4514 | 0.5135 |
