...
test.describe('desktop, listening', () => {
  ...
  test('MOMO\'s steps: one footfall at a time while running, none standing or in the air, and no pages turning', async ({ page }) => {
    ...
    await page.locator('[data-game-start]').click()
    await expect(page.locator('[data-game-overlay]')).toBeHidden({ timeout: 6000 })
    // A name ending in `_` counts the family: MOMO's three footsteps.
    const count = (n: string): Promise<number> => page.evaluate((n) => (window.__sfx ?? []).filter((s) => (n.endsWith('_') ? s.startsWith(n) && /\d$/.test(s) : s === n)).length, n)
    await expect.poll(async () => (await at(page)).x, { timeout: 3000 }).toBe(30)
    await page.waitForTimeout(1000)
    expect(await count('run_'), 'steps while standing').toBe(0)
    ...
    const before = await count('run_')
    // Carrying, running: footfalls at the walk's cadence, about five a second.
    await page.keyboard.down('ArrowRight')
    await page.waitForTimeout(1200)
    await page.keyboard.up('ArrowRight')
    const ran = (await count('run_')) - before
    const kinds = await page.evaluate(() => new Set((window.__sfx ?? []).filter((s) => /^run_\d$/.test(s))).size)
    expect(kinds, 'the three footsteps are not taken in turn').toBe(3)
    expect(await count('crew_step'), 'the crew\'s footstep in the game').toBe(0)
    ...
    const still = await count('run_')
    await page.waitForTimeout(1000)
    expect(await count('run_')).toBe(still)
    // A jump from standing is the jump's own sound, once, and no steps in the air.
    await page.keyboard.press(' ')
    await page.waitForTimeout(900)
    expect(await count('run_'), 'steps in the air').toBe(still)
